...
| Odontolaveglia · Clínica Odontológica (@odontolaveglia) · Nueva Barcelona, Anzoátegui | `odontolaveglia` | **Base OSCURA CÁLIDA** (espresso/taupe `#181310`/`#201814`, superficies `#271D18`/`#2E221C`) **+ acto en CREMA claro `#F5ECE3`** (sección "El trato") · **acento primario CORAL `#EE9B78`/`#F6BE9B`** — CTAs, títulos, retículas, y el escáner + su nube de puntos brillan en **durazno** (no azul frío); gold `#E8C07A` micro-acento · tinta crema `#F7EEE6` · amarillo Cashea `#FBF940` solo badge. ⚠️ **Deriva DELIBERADA del azul/navy de marca hacia esquema cálido navy+coral por feedback de la clienta** (v1 navy+cian "se sentía fría/tech"); la marca real es navy+cielo → si pide fidelidad exacta a su azul, se revierte la temperatura conservando el resto | **Epilogue** (display/UI 300–800) + **Space Mono** (eyebrows/HUD/datos) · densidad media-técnica clínica · base oscura | **Interfaz de escáner clínico / "estudios" numerados EST.01–06** — no bandas apiladas, no bento | **Canvas de nube de puntos "escaneo intraoral 3D"**: arcada dental (U) que se revela bajo un haz de escaneo que barre, con HUD "VISTA INTRAORAL · MAXILAR" y % en vivo (calco de su Shining 3D real) + **`@property` aura cónica** girando tras el visor + **tilt 3D** de la foto del escáner | **Barra de estado con píldora "Abierto ahora" EN VIVO** (calculada de su horario real: verde abierto / ámbar emergencias / cerrado-abre) / **Footer = panel de horario semanal** con día actual resaltado en vivo + emergencias en amarillo | Tarjetas "estudio" con **corchetes de retícula de escáner** (esquinas [ ]) + código mono EST.0X, hairlines · CTA principal = **rectángulo sky con underglow** · secundario = ghost hairline · constructor **"Arma tu cita"** (chips servicio+día+Cashea → ficha de cita → `wa.me` pre-armado) · FAB WhatsApp | 2026-07-27 | demo construida — **mensaje NO enviado** (Edith revisa) |
